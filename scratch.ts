...
async function run() {
  try {
    ...
    const newRunId = await sessionService.finishPage(
      ...
      "hello world",
      [
        { fromKey: null, toKey: "h", latencyMs: 100, keyChar: "h", holdDurationMs: 50, isCorrect: true, expectedChar: "h" },
        { fromKey: "h", toKey: "e", latencyMs: 100, keyChar: "e", holdDurationMs: 50, isCorrect: true, expectedChar: "e" },
      ],
      Date.now() - 200,
      Date.now()
    );
