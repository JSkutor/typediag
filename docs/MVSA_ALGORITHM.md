...
### 6.1. 인덱스 필드와 UI 커서 렌더링 활용
- **`targetIndex`**: 사용자가 입력한 내용이 목표 문장(`targetText`)의 몇 번째 글자에 대응하는지 매핑하는 데 사용됩니다. 이를 통해 각 글자별 상태 색상이나 누락(OMIT) 표시를 정확한 목표 글자 위치 위에 오버레이할 수 있습니다.
- **`inputIndex`**: QWERTY 물리 버퍼 내의 문자 인덱스를 나타냅니다. UI는 이 값을 기반으로 실시간 **타이핑 커서(typing-cursor)**의 위치를 동적으로 결정합니다.
  - 버퍼에 입력이 존재하는 경우 (`qwertyBuffer.length > 0`), 정렬 결과 배열 중 `inputIndex`를 갖는 마지막 요소의 오른쪽에 커서를 표시합니다.
  - 버퍼가 비어 있는 경우 (`qwertyBuffer.length === 0`), 첫 번째 문자 요소의 왼쪽에 커서를 표시합니다.
  - 이를 통해 한글 조합 도중이나 오타 복구 중에도 항상 실제 입력 지점에 맞춘 자연스러운 커서 흐름을 구현합니다.
