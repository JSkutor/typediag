...
| 고정 | **Vercel** | 월 정액 (한계 초과 시) | Next.js SSR·API 라우트 호스팅 |
| 고정 | **DB 호스팅** | 월 정액 (GCP Free → Hetzner/Supabase) | PostgreSQL + pgvector |
...
**발생 비용:** DB 호스팅(고정) + 스토리지·쓰기 RPS(변동, §5) + Vercel SSR/API 호출(§6.1).
...
전체 DB 크기는 코퍼스·인덱스·WAL을 포함하므로 $\Delta GB_{mo}$보다 큽니다. GCP Free cap(30GB) 비교에는 `pg_database_size` 사용.
...
배포 아키텍처 정본: `src/lib/dev/platformScaling.ts` (`GCP_FREE_TIER`, `VERCEL`, `HETZNER_VPS`).
...
### 6.2. Backend & DB — GCP Free Tier → Hetzner Cloud (또는 Supabase)

**1단계: GCP 평생 무료 티어 (e2-micro + Docker PostgreSQL)**

| 리소스 | 한도 |
| :--- | :--- |
| Compute | 2 vCPU (공유 코어) / 1 GB RAM (us 리전 한정) |
| 스토리지 | 30 GB (표준 영구 디스크) |
| 아웃바운드 | 월 200 GB (표준 인터넷 이그레스) |
| 비용 | **$0** |
...
**2단계: Hetzner Cloud VPS 또는 전문 Managed DB** (트래픽 스파이크 발생 시)
...
$$
\begin{aligned}
C_{fixed} &= C_{vercel} + C_{db\_host} + C_{clerk\_base} \\
C_{variable} &= C_{openai} + C_{upstage\_query} + C_{upstage\_passage} + C_{clerk\_overage} \\
C_{total} &= C_{fixed} + C_{variable}
\end{aligned}
$$

`db-disk` 증분은 호스팅 플랜·추가 볼륨에 이미 반영되거나 GCP cap 모니터링용이며, 별도 USD line item은 $0입니다.
...
| 단가 | OpenAI·Upstage·Clerk·Vercel·Hetzner/GCP 청구서 |
