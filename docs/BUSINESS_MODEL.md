...
| 고정 | **DB 호스팅** | 월 정액 (GCP Free → Hetzner/Supabase) | PostgreSQL + TimescaleDB + pgvector |
...
**1단계: GCP 평생 무료 티어 (e2-micro + Docker TimescaleDB-ha)**
...
| 쿼리 복잡도 | pgvector 유사도 검색 + TimescaleDB 집계 동시 발생으로 인한 OOM (Memory 부족) |
| 평균 쓰기 RPS | > 50 |
| DB 스토리지 | ≥ 30 GB |
