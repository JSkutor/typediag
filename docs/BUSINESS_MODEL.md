...
| :--- | :--- | :--- | :--- |
| 변동 | **Gemini** | 토큰당 (API 호출) | Topic Mode 문장 생성 |
...
Topic Mode는 **검색(Upstage) + 필요 시 생성(Gemini)** 이 핵심 변동비입니다.
...
주제 입력 (Enter)
  ...
  [유사도 > 0.5 행 없음] → 404
       → POST /api/practice/topic/generate
            → Gemini generateContent 1회 (문장 20개 JSON)
...
### 3.1. Gemini (Topic 문장 생성)

**호출 위치:** `src/lib/api/topicGenerateGemini.ts` → `POST /api/practice/topic/generate`
...
**공식 단가 (Paid tier, USD / 1M tokens)** — `GEMINI_PRICING`:
...
$$
C_{gemini} = m_{retry} \times \left( \frac{T_{in}}{10^6} P_{in} + \frac{T_{out}}{10^6} P_{out} \right)
$$

- $m_{retry}$: 재시도·폴백 반영 계수 (계획값 **1.05**)
- $T_{in}, T_{out}$: input/output 토큰 수
- $P_{in}, P_{out}$: 선택 모델 단가

**토큰 수 추정 (`estimateTopicGeminiTokens`):**

프롬프트 정본 `src/lib/practice/prompts.json` + `topicGenerateGemini.ts`와 동일하게 구성합니다.
...
$$
C_{gemini,mo} = N_{generate} \times C_{gemini}
$$
...
- Gemini 1회 생성 → 20행 적재 → 배치 1회분 passage 토큰 ≈ 20문장 join 기준 추정

Topic 런타임 생성 직후에는 embedding이 NULL이므로, **배치 전까지는 동일 주제 재검색이 404 → Gemini를 다시 탈 수 있음**. 코퍼스 운영 주기가 Gemini 비용에 간접 영향을 줍니다.
...
| `freeGeminiCapPerMauMonth` | 5 | 무료 MAU 1인당 월 Gemini 상한 |
...
D_{gemini,free} &= N_{search,free} \times \bar{g} \\
N_{generate,free} &= \min(D_{gemini,free},\ M_{free} \times 5) \\
...
- 차단된 무료 Gemini = $\max(0,\ D_{gemini,free} - N_{generate,free})$
...
C_{variable} &= C_{gemini} + C_{upstage\_query} + C_{upstage\_passage} + C_{clerk\_overage} \\
...
| 비용이 커지는 조건 | 이유 |
| :--- | :--- |
| Topic Mode 이용 비율 ↑ | Upstage query·Gemini 호출 증가 |
| 코퍼스 작음 / 배치 임베딩 지연 | 캐시 미스·얇은 히트 → Gemini 반복 |
...
| Gemini·Upstage 호출 수 | 프로덕션: API 로그·벤더 대시보드 (코드 미집계) |
| 단가 | Google AI Studio·Upstage·Clerk·Cloudflare·Hetzner 청구서 |
