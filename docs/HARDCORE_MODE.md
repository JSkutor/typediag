...
> 사용자의 실제 타건 이벤트를 영속성 계층(PostgreSQL) 또는 Zustand 상태 스토어에서 조회하여 취약 키(Weak Keys)를 계산하는 연동 로직은 아직 완성되지 않았습니다. 현재 `src/lib/practice/hardcoreModel.ts`의 `getUserWeakKeys()` 함수는 빈 배열(`[]`)을 반환하는 스텁 상태로 구현되어 있으며, 추후 실제 사용자 타건 지연 시간/오타율 기반 통계 가중치와 연동될 예정입니다.
