...
### 1.5. Subject Mode Fetching & State (`useSubjectTargets`)

- **Purpose**: Manages the local pool of generated/fetched sentences specifically for Subject Mode, providing an infinite-scroll-like experience.
- **Key State / Mechanism**:
  - Maintains an array of `targets` fetched from `/api/practice/subject`.
  - When the user types through the sentences and the remaining count in the pool drops below a threshold (e.g., `< 3`), it triggers `fetchMoreSubjectTargets()`.
  - `fetchMoreSubjectTargets()` calls the LLM fallback (`/api/practice/subject/generate`) passing the `existingSentences` array to prevent duplicate sentences from being generated by Gemini.
  - This state is strictly isolated from the generic `createInputSlice.ts` to keep the core typing store agnostic of the specific mode's data-fetching strategies.
