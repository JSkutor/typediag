...
*   **Key Actions**:
    *   `initializeRun(now)`: Resolves or triggers a new run ID via `SessionService`.
...
### 2.1. 5-Minute Inactivity (Idle) Rule
...
*   If the gap since the last active page or run start is **greater than 5 minutes**, the current run is finalized (status set to `completed`), and a new `run` is created.
*   If the gap is **less than 5 minutes**, the user continues within the same `run`.

### 2.2. 10-Minute Gap Splitting (Intra-page) Rule
If a user pauses mid-sentence for a very long time (e.g., gets up for a coffee) and finishes the sentence later:
*   If the elapsed time for a single page exceeds **10 minutes**:
    ...
    4.  The starting timestamp of this page is adjusted to prevent a massive 10-minute gap from destroying the WPM calculation (see *Time Correction* below).
...
*   **Gap Time Correction**: After a gap of 5+ minutes mid-sentence, the starting timestamp of the page is corrected using:
    \[
