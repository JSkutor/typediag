...
flowchart TD
    ...
    --> Step2["2. 이상치 필터링 (filter_outliers)<br>2000ms 하드 컷오프 선적용<br>1500건 미만: 정적 차단 유지<br>1500건 이상: 로그 IQR(최소 500ms 보장) 동적 차단"]:::step
...
- **로직**: 
  1. 먼저 `2000ms`를 넘는 극단적 지연을 무조건 제거(하드 컷오프)합니다.
  2. 남은 데이터가 1500건 미만일 때는 하드 컷오프 기준 내에서 최댓값을 상한으로 사용합니다.
  3. 데이터가 1500건 이상 쌓이면 로그 변환 후 IQR 임계값($Q_3 + 1.5 \times \text{IQR}$)을 계산하여 동적으로 이상치를 필터링합니다. 단, 임계값이 너무 낮아지는 것을 막기 위해 최소 `500ms`를 하한선 가드로 보장합니다.
