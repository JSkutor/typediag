 ...
 */
export function assertSentenceCompletion(
  target: string,
  typed: string,
  mvsaArray: AlignResult[],
): AssertionResult {
  // 타겟과 타이핑한 문장이 완벽히 일치하는데 mvsaArray에 REPLACE/INSERT가 남아있는지
  if (target === typed) {
    const hasErrorMark = mvsaArray.some((item) => item.op === "REPLACE" || item.op === "INSERT");

    if (hasErrorMark) {
      return {
        passed: false,
        reason:
          "False Positive: Target and Typed are identical, but MVSA output has REPLACE/INSERT states.",
      };
    }
  }
