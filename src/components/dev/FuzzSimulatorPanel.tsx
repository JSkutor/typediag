...
export function FuzzSimulatorPanel({ targets }: { targets: string[] }) {
  ...
  return (
    <div className="flex flex-col w-full max-w-5xl gap-6 p-6 mx-auto mt-10" style={{ color: "var(--text-primary)" }}>
            ...
            <div className="bg-black/50 p-4 rounded font-mono text-xs whitespace-pre-wrap max-h-64 overflow-y-auto">
              {crashLog.logs.slice(-10).map((log, i) => (
                <div key={i} className="flex gap-4 mb-1">
                  ...
                  <span className={`w-20 ${log.action.type === 'NORMAL' ? 'text-green-400' : 'text-red-400'}`}>
                    [{log.action.type}]
                  </span>
                  <span className="w-24">Key: {log.action.char}</span>
                  <span className="opacity-60">Cursor: {log.cursorBefore} &rarr; {log.cursorAfter}</span>
                </div>
              ))}
            </div>
          </div>
