  ...
  return (
    <div className="typing-area" style={{ width: "100%", textAlign: "center", fontSize: "1.875rem", fontFamily: "var(--font-mono)", lineHeight: 1.625 }}>
      <div id="typing-text-container" className="typing-text-container inline-block text-left" style={{ maxWidth: "1024px" }}>
        {diffResult.length === 0 && (
          <span className="typing-cursor left" />
        )}
        {diffResult.map((d, i) => {
          ...
          else if (d.op === "INSERT" || d.op === "REPLACE") highlightClass = "text-char-error opacity-80";
          ...
          return (
            <span key={i} id={`text-char-${i}`} className="text-char-container relative">
              {d.op !== "INSERT" && (
                <span className={`text-char-muted ${isMissing ? "border-b-4 border-red-500/30" : ""}`}>
                  {d.targetChar === " " ? "\u00A0" : d.targetChar}
                </span>
              )}
              
              {(d.op === "EQUAL" || d.op === "PARTIAL" || d.op === "REPLACE" || d.op === "INSERT") && (
