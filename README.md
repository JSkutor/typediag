...
| 환경 변수 | 용도 및 설명 |
| :--- | :--- |
| `DATABASE_URL` | 세션 저장 및 토픽 벡터 캐싱을 위한 PostgreSQL 연결 URI |
...
```bash
npm run db:up      # Docker를 통한 PostgreSQL 컨테이너 실행
...
*   [**DB 스키마 & 수명 주기 (DB_SCHEMA.md)**](docs/DB_SCHEMA.md) — DB 스키마 및 세션 관리 전략
